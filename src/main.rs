// https://github.com/rp-rs/rp-hal/blob/main/rp2040-hal-examples/src/bin/gpio_irq_example.rs
...
// Requires the rp2040::rt feature.
...
fn main() -> ! {
    ...
    critical_section::with(|cs| {
        GLOBAL_PINS.borrow(cs).replace(Some((led, button)));
    });
    unsafe {
...
fn IO_IRQ_BANK0() {
    ...
    if let Some((led, button)) = LED_AND_BUTTON
        && button.interrupt_status(EdgeHigh)
    {
        let _ = led.toggle();
